...
/**
 * PX Plugin "multiUAPublish"
 */
class pxplugin_multiUAPublish_register_pxcommand extends px_bases_pxcommand{
...
<?php
		...
		if(!is_dir($this->path_docroot_dir)){
			...
			$src .= '	<div><input type="hidden" name="PX" value="plugins.multiUAPublish.run" />'.(strlen( $this->param_path_target )?'<input type="hidden" name="path_target" value="'.t::h( $this->path_target ).'" />':'').'</div>'."\n";
			$src .= '	</form>'."\n";
			$src .= '</div><!-- /.unit -->'."\n";
			...
			$src .= '       	<dd>$ curl '.t::h($auth_curl).''.t::h( t::data2text('http://'.$_SERVER['HTTP_HOST'].$this->path_target.'?PX=plugins.multiUAPublish.run'.(strlen( $this->param_path_target )?'&path_target='.urlencode( $this->path_target ):'') ) ).'</dd>'."\n";
			$src .= '		<dt>"wget" コマンドが使える場合</dt>'."\n";
			$src .= '       	<dd>$ wget '.t::h($auth_wget).''.t::h( t::data2text('http://'.$_SERVER['HTTP_HOST'].$this->path_target.'?PX=plugins.multiUAPublish.run'.(strlen( $this->param_path_target )?'&path_target='.urlencode( $this->path_target ):'') ) ).'</dd>'."\n";
			$src .= '   </dl>'."\n";
			$src .= '</div><!-- /.topic_box -->'."\n";
			$src .= ''."\n";
	...
	/**
	 * multiUAPublishカスタム：単体の動的コンテンツをパブリッシュする
	 */
